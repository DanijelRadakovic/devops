...
- Ingress resurs zavisi od samog Kubernetes klastera gde se hostuje. Drugačije se ponaša na GKE, EKS, k3d, Minikube itd.
...
- Postoji Ingress operator koji radi kreiranje i konfiguraciju za vas.
...
== ConfigMap - konfiguracioni fajlovi, `subPath` koncept
...
spec:
  containers:
    - name: app
      image: alpine
      command: ["/bin/app"]
      args: ["--config /etc/booking/config.toml"]
      volumeMounts:
      - name: config
        mountPath: /etc/booking/config.toml # Putanja unutar kontejnera
        subPath: dev.toml  # Ključ (fajl) iz ConfigMap-e
  volumes:
    - name: config
      configMap:
        name: booking
...
== ConfigMap - konfiguracioni fajlovi, `items` koncept
...
spec:
  containers:
    - name: app
      ...
      volumeMounts:
      - name: config
        mountPath: /etc/booking # --> /etc/booking/config.toml 
  volumes:
    - name: config
      configMap:
        name: booking
        items:
          - key: "dev.toml" # Ključ (fajl) iz ConfigMap-e
            path: "config.toml" # Naziv fajla u kontejneru
```

== ConfigMap - automatsko ažuriranje

- Mounted CoinfigMap-e se #link("https://kubernetes.io/docs/concepts/configuration/configmap/#mounted-configmaps-are-updated-automatically")[automatski ažuriraju] kada nastanu izmene. Samim tim nema potrebe da se restartuje Pod.

- Ukoliko se koristi `subPath`, *gubi se* mehanizam automatskog ažuriranja, dok korišćenje `items` omogućava automatsko ažuriranje.

- Aplikacije koje hoće da koriste ovaj mehanizam moraju da implementiraju u pozadini tred koji će gledati promene fajla.

- *Napomena*: ukoliko se podesi `mountPoint` bez `subPath`-a, sav sadržaj folder unutar kontejnera će biti pregažen i unutar tog folder će se nalaziti fajlovi definisani u ConfigMap-i.  

== ConfigMap - primer automatskog ažuriranja
  ...
  containers:
    - name: alpine
      ...
      volumeMounts:
        - name: config
          mountPath: /etc/booking
  volumes: [{"name": "config", "configMap": {"name": "booking"}}]
```
...
kubect  exec -it demo -- ash
watch -n 1 "ls -l /etc/booking && echo --- && cat /etc/booking/dev.toml"
```

== ConfigMap - primer automatskog ažuriranja

```
Every 1.0s: ls -l /etc/booking && echo --- && cat /etc/booking/dev.toml               2026-04-19 17:32:06

total 0
lrwxrwxrwx    1 root     root            15 Apr 19 17:27 dev.toml -> ..data/dev.toml
---
mode=dev
timeout=1m
```
...
```bash
kubect edit configmap booking
```

== ConfigMap - primer automatskog ažuriranja

```
Every 1.0s: ls -l /etc/booking && echo --- && cat /etc/booking/dev.toml                2026-04-19 17:37:44

total 0
lrwxrwxrwx    1 root     root            15 Apr 19 17:27 dev.toml -> ..data/dev.toml
---
mode=prod  <---- changed this line
timeout=1m
```

== ConfigMap - automatsko ažuriranje

- Postoje latency lag prilikom ažuriranja na koji treba obratiti pažnju.

- Na letency utiču 3 stvare:
  - Propagation Delay: Vreme koje je potrebno da Kubernetesov API server notifikuje ostale komponente klastera da se izmena ConfigMap-e desila.
  - Kubelet Sync Period: Ovo je najveći lag. Kubelet na svakog nodu izvršava sync loop (podrazuvemano 1 minut) kako bi proverio da li se trenutni volume podudara sa željenim stanjem.
  - Cache TTL: Kubelet ponekad ne komunicira direktno sa API serverom, jer koristi lokalni keš i može se konfigurisati pomoću `configMapAndSecretChangeDetectionStrategy`.

- Može se koristiti u nekritičnim komponentama sistema. Za kritične komponente, koristiti klasičan način ažuriranja.

== ConfigMap

- ConfigMap-a i Pod moraju da budu u istom namepsace-u. Pod ne može koristiti ConfigMap-e iz drugog namespace-a.

- Ukoliko Pod treba da koristi ConfigMap-u iz drugog namespace-a, ConfigMap-a se mora kopirati u namespace Pod-a.
...
- Za vizualizaciju objekata unutar klastera može se koristiti: #link("https://headlamp.dev/")[Headlamp]
